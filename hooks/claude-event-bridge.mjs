...
const STATE_FILE = process.env.CLAUDE_NOTIFIER_STATE_FILE ?? `${process.env.HOME}/.claude-raycast-notifier/state.json`;
const MAX_RECENT = Number(process.env.CLAUDE_NOTIFIER_MAX_RECENT ?? "10");
const VOICE_EVENTS = new Set((process.env.CLAUDE_NOTIFIER_VOICE_EVENTS ?? "needs_input,failure,done").split(","));
...
if (VOICE_EVENTS.has(event.type)) {
  ...
  const voiceState = (await readState(STATE_FILE)).voice ?? { lastSpokenAt: null, lastSpokenText: null };

  if (shouldSpeak(voiceState, text, 15_000)) {
    await speak(text);
    await writeVoiceMeta(STATE_FILE, { lastSpokenAt: Date.now(), lastSpokenText: text });
  }
}
